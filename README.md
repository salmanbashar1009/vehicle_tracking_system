...
2. Firebase Setup
   ...
   Place the config file in the appropriate directory (android/app/ for Android).
3. Installation
# Clone the repositorygit clone <repo-url># Navigate to project directorycd vehicle_tracking_system# Install dependenciesflutter pub get
4. Seed Dummy Data
   To test the app immediately with simulated vehicles:

Temporarily add a call to seedDummyData() inside main.dart right after configureDependencies().
Run the app once.
Remove the seed call and hot restart. The database now has data!
5. Run the App
   bash
...
Markers change color based on tracking status (Green = Live, Grey = Offline).
Bottom sheet displays vehicle & driver details on marker tap.
Auto-centers on vehicle when requested.
📡 Tracking Simulation
...
Adds random variance (±0.0005) to lat/lon to simulate movement.